...
export class SwapHandler {
  ...
  private _handle(token: Address, amountIn: BigInt): void {
    ...
    const generatedFees = this._getGeneratedFees(amountIn)
    const maxRewardAmount = this._getAmount(
      generatedFees,
      this.pairFactory.MAX_REFERRAL_FEE()
    )

    // take 5% of the max reward amount and add it to the dibs acc balance
    const dibsCut = maxRewardAmount
      .times(this.dibs.dibsPercentage())
      .div(this.dibs.SCALE())

    const remainingRewardAmount = maxRewardAmount.minus(dibsCut)
    ...
    this._accPlatform(remainingRewardAmount, token, AccType.DEPOSIT)
    ...
    if (this._isRegisteredUser()) {
      ...
      this._distributeRewards(remainingRewardAmount, token)
  ...
  private _accPlatform(amount: BigInt, token: Address, accType: AccType): void {
    const id = token.toHex() + "-PLATFORM"
    let accPlatform = AccumulativeTokenBalance.load(id)

    if (accPlatform == null) {
      accPlatform = new AccumulativeTokenBalance(id)
      accPlatform.token = token
      accPlatform.amount = amount
      accPlatform.save()
    }

    if (accType == AccType.DEPOSIT) {
      accPlatform.amount = accPlatform.amount.plus(amount)
    } else {
      accPlatform.amount = accPlatform.amount.minus(amount)
    }

    accPlatform.lastUpdate = this.timestamp

    accPlatform.save()
  }

  private _distributeRewards(
    ...
    const referrerAmount = rewardAmount.times(referrerPercentage).div(scale)
    const refereeAmount = rewardAmount.times(refereePercentage).div(scale)
    ...
    addAccumulativeTokenBalance(token, this.user, refereeAmount, this.timestamp)
  }
  ...
  private _getAmount(amount: BigInt, percentage: BigInt): BigInt {
    const rewardAmount = amount.times(percentage).div(this.dibs.SCALE())
    return rewardAmount
  }
